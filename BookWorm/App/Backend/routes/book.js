...
router.post("/add", (request, response) => {
  const { title, author, price, sellerid,imagepath } = request.body;

  const statement = `
    INSERT INTO ${BOOK_TABLE} (title, author, price, sellerid,imagepath)
    VALUES (?,?,?,?,?)
  `;

  pool.execute(
    statement,
    [title, author, price, sellerid,imagepath],
    (err, result) => {
      if (err) {
        response.send(utils.createError(err.message));
      } else {
...
router.get("/:id", (request, response) => {
  ...
  const statement = `SELECT * FROM ${BOOK_TABLE} WHERE bookid =?`;
